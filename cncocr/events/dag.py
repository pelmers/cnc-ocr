...
class DAG(object):
    ...
    def __init__(self, nodes = None):
        '''
        Init the graph with given nodes, a dict mapping
            {node_id: {children}}
        '''
    ...
    def transpose(self):
        '''
        Return the transpose (reversed edges) of the graph
        '''
    ...
    def bfs(self, start_node = None, visitor = lambda x: x):
        ...
        traveling along edges connecting parents to children.
        Perform visitor(node) on each visited node.
    ...
    def collect_leaves(self, node_id):
        '''
        Collect all leaves below the node for node_id,
        returning a list of ids for these leaves
        '''
        leafs = []
